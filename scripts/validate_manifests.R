...
manifest_dir <- file.path(example_root, "assessment-years", "2024")
manifest_specs <- list(
...
for (manifest_name in names(manifest_specs)) {
  ...
  if (!file.exists(manifest_path)) {
    failures <- c(failures, paste("Missing", manifest_name))
    next
  }
  ...
  if (length(missing_columns) > 0) {
    failures <- c(
      failures,
      paste(manifest_name, "missing columns", paste(missing_columns, collapse = ", "))
    )
  }

  first_column <- names(manifest)[1]
  if (anyNA(manifest[[first_column]]) || any(manifest[[first_column]] == "")) {
    failures <- c(failures, paste(manifest_name, "has a blank identifier"))
  }
  if (anyDuplicated(manifest[[first_column]]) > 0) {
    failures <- c(failures, paste(manifest_name, "has duplicate identifiers"))
  }
}

assessment_path <- file.path(manifest_dir, "assessment.yml")
if (!file.exists(assessment_path)) {
  failures <- c(failures, "Missing assessment.yml")
}
